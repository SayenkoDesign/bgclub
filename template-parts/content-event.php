...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'column row' ); ?>>
    ...
    if( !is_single() ) {
        ...
        $event_date = sprintf( '<h3>Event Date: %s</h3>', get_field( 'event_start_date' ) );
    
        printf( '<div class="post-hero" style="%s"><div class="flex"><header class="entry-header">%s%s%s%s</header></div></div>', 
                $post_image, $post_categories, $post_title, $event_date, $post_author );
